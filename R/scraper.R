...
#' @title voting URL
votes_url <- function(legislatura, ano, periodo, tipo) {
  fields <- paste0(c("sm", "ano", "tp", "np", "lg"),
                   "=",
                   c(1, ano, tipo, periodo, legislatura))
  paste(c(base_votes_url(), fields), collapse = "&")
}

#' @title download a period
download_period <- function(legislatura, ano, periodo, tipo) {
  base_votes_url() %>%
    rvest::html_session() %>%
    rvest::follow_link(texto) %>%
    xml2::read_html()
}
